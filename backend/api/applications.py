...
# ============================================================================
# FILE BROWSER ENDPOINTS - Simple upload without auto-processing
# ============================================================================
...
async def upload_files_only(
    ...
    """
    Upload files WITHOUT automatic processing (no LLM, embeddings, or Elasticsearch).
    Files are stored and marked as not indexed.
    Use POST /documents/{id}/index to manually index later.
    """
    ...
    if application_id:
        ...
        raise HTTPException(status_code=400, detail="Either application_id or company_name required")
    ...
    for file in files:
        ...
        content = await parser.parse_document(file)
        ...
        doc = ApplicationDocument(
            ...
            content=content,
            doc_type=None,  # Not classified yet
            indexed=False  # Mark as not indexed
        )
        db.add(doc)
        db.flush()
    ...
    return {
        ...
        "message": "Files uploaded. Use context menu to index when ready."
    }
...
async def index_document(
    ...
    """
    Manually index a document: LLM classification + embeddings + Elasticsearch.
    This is triggered by right-click menu "Indexieren" in file browser.
    """
    # Get document
    ...
    # Verify ownership
    app = db.query(Application).filter(
        Application.id == doc.application_id,
        Application.user_id == user.id
    ).first()
    ...
    try:
        ...
        doc_type = await classify_document_with_llm(doc.content[:1500], doc.filename)
        ...
        return {
            ...
            "doc_type": doc_type,
            "indexed": True,
            "steps_completed": ["LLM Classification", "Embeddings", "Elasticsearch"]
        }
...
async def list_files(
    ...
    """
    List all uploaded files with their indexing status.
    For file browser display.
    """
