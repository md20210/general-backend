...
async def upload_files_only(
    ...
    Files are uploaded and stored, but NOT indexed (no LLM, no embeddings, no Elasticsearch).
    User can manually trigger indexing later via context menu.
    """
    if not files:
        raise HTTPException(status_code=400, detail="No files provided")
    ...
    # If application_id provided, use existing application
    if application_id:
        app = db.query(Application).filter(
            Application.id == application_id,
            Application.user_id == user.id
    ...
    for uploaded_file in files:
        try:
            ...
            # Extract text but DON'T process it
            extracted_text = await parser.parse_file(display_filename, file_data)

            # Save document WITHOUT indexing (indexed=False)
            document = ApplicationDocument(
...
async def index_document_manually(
    ...
    This performs the full processing that was skipped during upload:
    1. Classify document type using LLM
    2. Generate embedding vector
    3. Index in Elasticsearch
    4. Mark as indexed=True
    """
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Elasticsearch indexing failed: {es_error}")
            # Continue anyway - Elasticsearch is optional
...
async def list_all_files(
    ...
    Returns files grouped by application, showing which are indexed (green) vs not indexed (gray).
    """
